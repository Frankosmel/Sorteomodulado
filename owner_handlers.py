...
def register_owner_handlers(bot: TeleBot):
    ...
    def handle_owner_selection(msg):
        ...
        if text.startswith("Gestionar "):
            ...
            kb = ReplyKeyboardMarkup(resize_keyboard=True)
            kb.row("👥 Ver participantes", "🏆 Ver top invitadores")
            kb.row("🔄 Reiniciar sorteo", "🗑️ Borrar lista de sorteo")
            kb.row("🌐 Cambiar zona horaria", "🔙 Salir")
